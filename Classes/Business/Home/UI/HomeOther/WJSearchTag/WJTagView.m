...
- (void)addTag: (WJTag *)tag {
    NSParameterAssert(tag);
    WJTagButton *btn = [WJTagButton buttonWithTag: tag];
...
- (void)insertTag: (WJTag *)tag atIndex: (NSUInteger)index {
    NSParameterAssert(tag);
...
{
    [hotKeyArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        // 初始化标签
        
        WJHotKeysModel *hotKeyModel = hotKeyArray[idx];

        WJTag *tag = [[WJTag alloc] initWithText:hotKeyModel.name];

        // 标签相对于自己容器的上左下右的距离
        tag.padding = UIEdgeInsetsMake(3, 15, 3, 15);
        // 弧度
        tag.cornerRadius = self.regularHeight/2;
        // 字体
        tag.font = [UIFont boldSystemFontOfSize:14];
        // 边框宽度
        tag.borderWidth = 0.5;
        // 背景
        tag.bgColor = WJColorWhite;
        // 边框颜色
        tag.borderColor = WJColorSeparatorLine;
        // 字体颜色
        tag.textColor = WJColorMainTitle;
        // 是否可点击
        tag.enable = YES;
        // 加入到tagView
        [self addTag:tag];
    ...
    // 获取刚才加入所有tag之后的内在高度
    CGFloat tagHeight = self.intrinsicContentSize.height;
    NSLog(@"高度%lf",tagHeight);
    self.frame = CGRectMake(0, 0, kScreenWidth, tagHeight);
    [self layoutSubviews];
    _totalHeight = tagHeight;
}
