...
@interface WJChargeOrderCell ()
{
    ...
    UIButton    *chargeAgainButton;     //再次充值
}

@end

@implementation WJChargeOrderCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        chargeAgainButton = [UIButton buttonWithType:UIButtonTypeCustom];
        chargeAgainButton.frame = CGRectMake(kScreenWidth - ALD(10) - ALD(90), bottomLine.bottom + ALD(10), ALD(90), ALD(30));
        [chargeAgainButton setTitle:@"再次充值" forState:UIControlStateNormal];
        [chargeAgainButton setTitleColor:WJColorMainColor forState:UIControlStateNormal];
        chargeAgainButton.layer.cornerRadius = 4;
        chargeAgainButton.layer.borderColor = WJColorMainColor.CGColor;
        chargeAgainButton.layer.borderWidth = 0.5;
        chargeAgainButton.titleLabel.font = WJFont14;
        chargeAgainButton.hidden = YES;
        [chargeAgainButton addTarget:self action:@selector(chargeAgainButtonAction) forControlEvents:UIControlEventTouchUpInside];
        ...
        [self.contentView addSubview:chargeAgainButton];
...
-(void)configDataWithOrder:(WJOrderModel *)order
{
    ...
    NSString *totalMoneyStr = [NSString stringWithFormat:@"合计: %@元",order.PayAmount];
...
-(void)chargeAgainButtonAction
{
    self.chargeAgainBlock();
}
