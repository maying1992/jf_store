...
- (void)layoutSubviews
{
    ...
    [[self buttons] enumerateObjectsUsingBlock:^(UIButton *button, NSUInteger idx, BOOL *stop) {
        ...
        CGRect rect = CGRectMake(roundf(self.bounds.size.width/self.numberOfSegments) * idx, 0, roundf(self.bounds.size.width/self.numberOfSegments), self.bounds.size.height);
...
- (UIColor *)titleColorForState:(UIControlState)state
{
    ...
    if (!color) {
        switch (state) {
            case UIControlStateNormal:              return WJColorDardGray3;
            case UIControlStateHighlighted:         return self.tintColor;
            case UIControlStateDisabled:            return WJColorDardGray3;
